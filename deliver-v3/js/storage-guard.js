...
//
// This module has NO network / DOM dependencies so the rules can be unit-tested
// in isolation. Usage numbers are NEVER computed here: they come from the Worker
// (GET /api/admin/storage). The Worker ALSO enforces the 3 GB cap atomically on
// every upload — this client-side check is only an instant, friendly pre-check.
...
export class StorageLimitError extends Error {
  constructor(kind, details = {}) {
    ...
    if (kind === "full") {
      ...
      body = `Boztik Deliver has reached its ${limit} storage limit, so uploads are locked. Delete old or expired deliveries to free space.`;
