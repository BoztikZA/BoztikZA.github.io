...
/* =========================================================
   PUBLIC-SITE EXIT MARKER
=========================================================

   The delivery page links back to the public website (../index.html).
   The public-site analytics beacon (js/site-analytics.js) must NOT count
   those navigations as new website visits — somebody delivering files (or a
   client who just landed on a delivery page) heading home is not fresh public
   traffic. So, just BEFORE such a link is followed, we drop a short-lived
   sessionStorage marker. The beacon sees it (or sees a /deliver/ referrer)
   and skips recording. This runs on the /deliver/ page only — the beacon
   never runs here, and the marker is only ever consumed by the beacon.
========================================================= */

function bindSiteExitMarker() {

  const SKIP_KEY =
    "boztik_skip_site_visit";

  const pointsToPublicSite =
    (anchor) => {
      ...
      try {
        ...
        if (
          /\bboztik\.com\b/i.test(
            url.hostname
          )
        ) {

          return !url.pathname.toLowerCase().startsWith(
            "/deliver"
          );
        ...
        /* relative-only link — handled by the checks above. */
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      try {
        ...
        /* Non-persistent contexts are fine — the /deliver/ referrer guard in
           the beacon still covers the navigation. */
...
async function init() {
  ...
  /* Flag any "Go to site" (home) link to the public site so the public-site
     analytics beacon does not count the hand-back as a new website visit. */
  bindSiteExitMarker();
