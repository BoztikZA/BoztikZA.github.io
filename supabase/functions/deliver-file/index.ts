// Boztik Deliver: securely issues short-lived URLs for files belonging to an active delivery.
...
Deno.serve(async request => {
  ...
  const { data: delivery, error: deliveryError } = await supabase.from("deliveries")
    .select("id, expires_at, file_path, storage_deleted_at").eq("id", input.deliveryId).maybeSingle();
  if (deliveryError) return json({ error: "lookup_failed" }, 500);
  if (!delivery) return json({ error: "delivery_not_found" }, 404);
  if (delivery.storage_deleted_at || new Date(delivery.expires_at).getTime() <= Date.now()) {
    return json({ error: "delivery_expired" }, 410);
  }

  let belongsToDelivery = delivery.file_path === input.filePath;
  if (!belongsToDelivery) {
    const { data: file } = await supabase.from("delivery_files").select("file_path")
      .eq("delivery_id", input.deliveryId).eq("file_path", input.filePath).maybeSingle();
  ...
  const download = input.mode === "download"
    ? { download: input.fileName || input.filePath.split("/").pop() }
    : undefined;
