...
// including the Photoshop Battles delivery type. Talks to the Boztik Deliver Worker only
// through auth.js / api.js / shared.js — no direct network calls here.
...
const els = {
  ...
  sortSelect: $("dash-delivery-sort"),
  insightsBody: $("dash-insights-body"),
...
function battleDirectUrl(delivery) {
  ...
  return `${config.apiBaseUrl}/api/public/photoshop-battles-image/${encodeURIComponent(delivery.id)}--${encodeURIComponent(token)}.${ext}`;
}
 ...
 *  beyond the loaded page of deliveries and after deliveries are deleted. */
let serverOverview = null;
function applyServerTotals(overview) {
  serverOverview = overview || serverOverview;
  const a = serverOverview?.activity;
  if (!a) return;
  if (els.statMonthlyViews) els.statMonthlyViews.textContent = a.month.views;
...
function renderOverview() {
  ...
  renderActivity();
  applyServerTotals(null); // prefer the Worker's aggregates once we have them
}
...
function compareDeliveries(a, b) {
  const [key, dir] = (els.sortSelect?.value || "created-desc").split("-");
  const value = d => key === "views" ? Number(d.view_count || 0)
    : key === "downloads" ? Number(d.download_count || 0)
    : key === "expires" ? new Date(d.expires_at).getTime()
...
function renderDeliveryCard(delivery) {
  ...
  card.innerHTML = `
    ...
    <div class="dash-delivery-card-actions">
      ...
      <button type="button" class="dash-btn dash-compact btn-analytics">Analytics</button>
  ...
  card.querySelector(".btn-analytics")?.addEventListener("click", () => void openDeliveryAnalytics(delivery));
...
function setupDeliveriesTab() {
  ...
  els.sortSelect?.addEventListener("change", renderDeliveryList);
  els.usageBody?.addEventListener("click", handleUsageAction);
...
function renderStorageUsageError(message) {
  renderUsageMessage(`
    ...
    <p class="dash-usage-muted">
      Uploads stay blocked until usage can be verified.
      <button type="button" class="dash-usage-action" data-usage-action="reconcile">Verify against storage</button>
    </p>
...
async function loadStorageUsage() {
  ...
  let result;
  try {
    result = await fetchStorageUsage();
  } catch (error) {
  ...
  if (!s || typeof s.used_bytes !== "number" || !s.limit_bytes) {
    renderStorageUsageError("The server did not return usable storage data.");
    return;
  }

  const pct = Math.min(100, s.used_percent ?? 0);
  const meterClass = s.level === "full" || s.level === "critical" ? "is-danger" : s.level === "warning" || s.level === "notice" ? "is-warning" : "";
  const level = (config.storageWarningLevels || []).find(item => pct >= item.minPercent) || { key: "healthy", label: "Healthy" };
  const stateClass = ["over_quota", "critical"].includes(level.key) ? "has-danger" : ["warning", "notice"].includes(level.key) ? "has-warning" : "";
  const files = Number(s.file_count || 0);
  const verified = s.last_reconciled_at ? formatDate(s.last_reconciled_at) : null;

  const remainingHtml = s.remaining_bytes > 0
    ? `${formatBytes(s.remaining_bytes)} remaining`
    : `<span class="dash-usage-exceeded">Storage limit reached — uploads locked</span>`;

  const notes = [];
  if (s.upload_locked) notes.push(`<p class="dash-usage-message">Uploads are locked until space is freed. Delete old deliveries or clean expired ones.</p>`);
  if (s.accounting_uncertain) notes.push(`<p class="dash-usage-message">Storage accounting is being re-verified; uploads pause until it is confirmed.</p>`);
  if (s.reserved_bytes > 0) notes.push(`<p class="dash-usage-muted">${formatBytes(s.reserved_bytes)} reserved by uploads in progress.</p>`);

  renderUsageMessage(`
    <div class="dash-usage-hero">
      <div class="dash-usage-hero-main">
        <span class="dash-usage-label">Storage used</span>
        <strong class="dash-usage-total">${formatBytes(s.used_bytes)}</strong>
        <span class="dash-usage-meta">${files.toLocaleString()} file${files === 1 ? "" : "s"}${verified ? ` · verified ${escapeHtml(verified)}` : ""}</span>
        <p class="dash-usage-source">Source: <strong>Boztik Deliver storage ledger</strong> · reconciled against R2</p>
      </div>
      <div class="dash-usage-meter ${meterClass}">
        <div class="dash-usage-meter-track"><span class="dash-usage-meter-fill" style="width:${pct}%"></span></div>
        <div class="dash-usage-meter-meta"><strong>${pct}%</strong><span>of ${formatBytes(s.limit_bytes)} hard cap</span></div>
        <dl class="dash-usage-rows">
          <div><dt>Limit</dt><dd>${formatBytes(s.limit_bytes)} <em>hard cap</em></dd></div>
    ...
    <div class="dash-usage-section">
      ${notes.join("")}
      <p class="dash-usage-actions">
        <button type="button" class="dash-usage-action" data-usage-action="cleanup">Clean expired deliveries</button>
        <button type="button" class="dash-usage-action" data-usage-action="reconcile">Verify against storage</button>
      </p>
    </div>
  `, level.label, stateClass);
}
  ...
  try {
    if (action === "cleanup") {
      ...
      for (let round = 0; round < 10; round += 1) {
        const r = await cleanExpiredDeliveries();
        cleaned += r.cleaned; freed += r.removed_bytes; failed += r.failed;
        if (!r.remaining || r.cleaned === 0) break;
      }
      showToast(failed
        ? `Cleaned ${cleaned} expired deliver${cleaned === 1 ? "y" : "ies"}; ${failed} could not be removed and will be retried.`
        : cleaned ? `Cleaned ${cleaned} expired deliver${cleaned === 1 ? "y" : "ies"} · freed ${formatBytes(freed)}.` : "Nothing to clean — no expired deliveries are waiting.", failed ? "error" : "success");
      await loadDeliveries();
    } else if (action === "reconcile") {
      const r = await reconcileStorage();
      showToast(r.reconcile.deferred
        ? "Uploads were in progress, so verification was postponed. Try again in a moment."
        : r.reconcile.drift_bytes ? `Storage corrected by ${formatBytes(Math.abs(r.reconcile.drift_bytes))}.` : "Storage verified — the ledger matches R2.", "success");
    }
  } catch (error) {
...
/* =========================================================
   ...
   guard (Worker GET /api/admin/storage). Never estimates in the
