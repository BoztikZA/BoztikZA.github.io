...
5. Apply the `20260908090000_preserve_expired_delivery_analytics` and `20260918120000_cleanup_access_hardening` lifecycle migrations (the latter stops the public page, signed URLs and storage reads from resolving any delivery whose files have been cleaned or manually deleted). Deploy `cleanup-expired-deliveries`, set a strong `CLEANUP_SECRET` function secret, and schedule a **30-minute** Supabase Cron job that invokes it — see `supabase/CLEANUP_SETUP.md` and `supabase/cron/jobs.sql`. The function removes only expired physical Storage objects and sets `deliveries.storage_deleted_at`; it does not delete delivery rows or analytics.
6. Apply the `20260918000000_storage_usage_monitor` migration, then deploy the usage monitor: `supabase functions deploy storage-usage --project-ref hwcxxotgtqchcriascti`. It feeds the Command Centre **Storage & usage** panel. Then, in `js/config.js`, set `storagePlanBytes` to the project plan's **configured** Storage allowance (Free = `1073741824`, Pro/Team = `107374182400`) to enable the quota meter + warnings. This value is entered manually — Supabase exposes no runtime API for the plan allowance, so the panel labels it "Configured plan allowance". Left `null`, the panel shows raw usage only and the allowance/remaining show as "Unavailable". The service-role key stays server-side; this function verifies the caller's sign-in session first.
